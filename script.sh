...
until [ "$selection" = "0" ]; do
	...
	case $selection in
		...
		3  ) 	curl -s -X GET -H 'Accept: application/json' http://localhost:8080/containers?state=running | python -mjson.tool
			press_enter
			;;
		4  ) 	curl -s -X GET -H 'Accept: application/json' http://localhost:8080/containers/97d73ac0656f | python -mjson.tool
			press_enter
			;;
		5  ) 	curl -s -X DELETE -H 'Accept: application/json' http://localhost:8080/containers/97d73ac0656f | python -mjson.tool
			press_enter
			;;
		6  ) 	curl -s -X DELETE -H 'Accept: application/json' http://localhost:8080/containersDel | python -mjson.tool
			press_enter
			;;
		7  ) 	curl -X POST -H 'Content-Type: application/json' http://localhost:8080/containers -d '{"image": "fb434121fc77"}' | python -mjson.tool
			press_enter
			;;
		8  )	curl -X PATCH -H 'Content-Type: application/json' http://localhost:8080/containers/fb434121fc77 -d '{"state": "running"}'
			press_enter
			;;
		9  ) 	curl -X PATCH -H 'Content-Type: application/json' http://localhost:8080/containers/fb434121fc77 -d '{"state": "stopped"}'
			press_enter
			;;
		10 )	curl -s -X GET -H 'Accept: application/json' http://localhost:8080/containers/97d73ac0656f/logs | python -mjson.tool 
			press_enter
			;;
